...
import { BlockHeader, SignedBlock, SignedBlockRawBlockApi } from '../chain/block';
...
export class DatabaseAPI {
    ...
    /**
     * Convenience for calling `block_api`.
     */
    public callBlock(method: string, params?: any) {
        return this.fetch.call(`block_api.${method}`, params);
    }
    ...
    public async getBlock(blockNum: number): Promise<SignedBlock | null> {
        const result: { block: SignedBlockRawBlockApi } = await this.callBlock('get_block', { block_num: blockNum });

        if (result?.block) {
            const block = result.block;
            const transactions = block.transactions.map((transaction, transactionIndex) => {
                const txId = block.transaction_ids[transactionIndex];
                return new SignedTransactionInBlock({
                    expiration: transaction.expiration,
                    extensions: transaction.extensions,
                    operations: transaction.operations.map((op) => {
                        return [op.type.replace('_operation', '') as OperationName, op.value];
                    }),
     ...
     * Return block *blockNum*.
     */
    public async getBlockCondenser(blockNum: number): Promise<SignedBlock> {
