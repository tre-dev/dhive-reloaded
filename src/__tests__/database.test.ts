...
describe('database api', function () {
    ...
    it('getBlock', async function () {
        ...
        if (result) {
            ...
            for (let i = 0; i < result.transaction_ids.length; i++) {
                const txOld = resultOld.transactions[i];
                ...
                console.log(`Transaction ${tx.transaction_id} has ${tx.operations.length} operations and matches with old api call`);
            }
        } else {
