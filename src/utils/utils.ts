...
export function log(msg: string, level: LogLevel = 0, color: LogColors | null = null) {
    ...
    if (level <= (process.env.LOGGING_LEVEL ? Number(process.env.LOGGING_LEVEL) : 5)) console.log(`${new Date().toLocaleString()} - ${msg}`);
}
...
export const isTxError = (err: Error) => {
    const cause: any = err?.cause;
    return err && err.name == 'RPCError' && cause && cause.code != 4030200 && ([10, 13].includes(cause.code) || cause.code > 1000000);
};
